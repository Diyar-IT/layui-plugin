  ...
  exports('code', function (options) {
    ...
    layui.each(options.elem.get().reverse(), function (index, item) {
      ...
      if (options.preview) {
        ...
        // 追加 Tab 组件
        var elemView = $('<div class="' + CONST.ELEM_PREVIEW + (options.skin ? ' ' + options.skin : '') + '">');
        ...
        elemToolbar.on('click', '>i', function () {
          ...
          tools[type] && layui.type(tools[type].event) === 'function' && tools[type].event(oi, type);
          options.toolsEvent && layui.type(options.toolsEvent) === 'function' && options.toolsEvent(oi, type);
        });
      ...
      }else if (othis.attr('lay-title') !== undefined && !options.preview){
        var skin = options.skin || othis.attr('lay-skin') || '';
        if (!othis.find('>.layui-code-h3')[0]) {
          othis.before('<div class="layui-code-custom-title layui-code-'+ skin +'">'
            + (othis.attr('lay-title') || options.title || '&lt;/&gt;') + '<a href="javascript:;">'
            + (othis.attr('lay-lang') || options.lang || '') + '</a>' + '</div>');
        }
      }
