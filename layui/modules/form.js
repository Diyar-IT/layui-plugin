    ...
    var items = {
      ...
      input: function (elem) {
        ...
        elemForm.find('input[lay-affix],textarea[lay-affix]').each(function () {
          ...
          var CLASS_TEXTAREA_COUNT = 'layui-textarea-count';
          var isTextarea = othis.is('textarea');
          var showCount = othis.attr('show-count') !== undefined;

          //if (isTextarea && affix !== 'clear') return false;
          // 根据是否空值来显示或隐藏元素
          ...
          var callChangeEvent = function (call, filter, obj) {
            layui.event.call(call, MOD_NAME, 'input-affix(' + filter + ')', obj);
          }
          ...
          var renderAffix = function (opts) {
            ...
            var elemIcon = $(function () {
              if (affix === 'text' || affix === 'isim' || affix === 'button') {
                var text = affix === 'isim' ? '\u2022'
                  : (othis.attr('affix-text') ? othis.attr('affix-text').substr(0, 2) : '*');
                return '<span class="affix-text">' + text + '</span>';
              }

              var arr = [];
              layui.each(value, function (i, item) {
                arr.push('<i class="' + (opts.diyaricon ? 'iconfont ' : 'layui-icon layui-icon-') + '' + item + (
            ...
            if (isTextarea && showCount){
              var hasCountElem = othis.parent().find('.' + CLASS_TEXTAREA_COUNT);
              var countVal = othis.attr('maxlength') ? othis.val().length + '/' + othis.attr('maxlength') : othis.val().length;
              var countElem = '<div class="'+ CLASS_TEXTAREA_COUNT +'">' + countVal + '</div>';
              if (hasCountElem[0]){
                hasCountElem.text(countVal);
              }else {
                othis.after(countElem);
              }
            }
            ...
            othis.on('input propertychange', function () {
              ...
              if (showCount) {
                var elem = $(this);
                elem.parent().find('.' + CLASS_TEXTAREA_COUNT)
                  .text(elem.attr('maxlength') ? value.length + '/' + elem.attr('maxlength') : value.length);
              }
            }).off('keypress').bind('keypress', function (event) {
              if (affix === 'number'){
            ...
            elemIcon.on('click', function (e) {
              ...
              // 对外事件
              callChangeEvent(this, inputFilter, {
                elem: othis[0],
                affix: affix,
                options: opts,
                event: e
            ...
            if (opts.roller && layui.isArray(opts.value)){
              othis.off('mousewheel DOMMouseScroll').on('mousewheel DOMMouseScroll', function (e) {
                opts.delta = (e.originalEvent.wheelDelta && (e.originalEvent.wheelDelta > 0 ? 0 : 1)) ||  // chrome & ie
                  (e.originalEvent.detail && (e.originalEvent.detail > 0 ? 1 : 0));// firefox

                var inputFilter = othis.attr('lay-filter');
                opts.click && typeof opts.click === 'function' && opts.click.call(this, othis, opts);
          ...
          var affixOptions = {
            ...
            clear: { // 内容清除
              ...
              click: function (elem) {
                ...
                if (elem.attr('show-count') !== undefined) {
                  elem.parent().find('.' + CLASS_TEXTAREA_COUNT)
                    .text(elem.attr('maxlength') ? elem.val().length + '/' + elem.attr('maxlength') : elem.val().length);
                }
              },
            ...
            number: { // 数字输入框
              ...
              click: function (elem, opts) {
                var index = $(this).index() || opts.delta;
                var value = elem.val();
                var rawValue = value;
                var step = Number(elem.attr('step')) || 1; // 加减的数字间隔
                var min = Number(elem.attr('min'));
                var max = Number(elem.attr('max'));
                var elemfixed = Number(elem.attr('fixed'));
                var moneyMode = elem.attr('money');
                var symbol = elem.attr('symbol');

                if (symbol) value = value.replace(symbol, '').replace(',','');
                ...
                if (fixed || elemfixed) value = value.toFixed(elemfixed || fixed);
                if (moneyMode !== undefined) value = layui.util.money_format(value, 2, symbol || '');

                elem.val(value);
              }
            },
            text: {
              className: 'layui-input-affix-text',
              disabled: disabled
            },
            button: {
              className: 'layui-input-affix-button',
              disabled: disabled
            },
            isim: {
              className: 'layui-input-affix-isim',
              disabled: disabled,
              click: function (elem) {
                var oldtext = elem.val();
                var pos = elem.get(0).selectionStart;
                if (!/[\u2022{1}]/gi.test(oldtext)) {
                  var newStr = oldtext.substring(0, pos) + '\u2022';
                  newStr += oldtext.substring(pos, oldtext.length - pos + newStr.length);
                  elem.val(newStr);
                }
              }
            }
  ...
  Form.prototype.validate = function (elem) {
    ...
    layui.each(elem, function (_, item) {
      ...
      layui.each(vers, function (_, thisVer) {
        ...
        var isMaxMin = (/^(max|min)\:\d{1,4}$/i.test(thisVer) && value.length > 0);
        var maxMinTips, maxMinArr;

        if (isMaxMin || isMaxMin) {
          maxMinArr = thisVer.split(':');
          maxMinTips = (othis.attr('lay-errtitle') || '此填项不能') + (maxMinArr[0].toLowerCase() === 'max' ? '超于' : '小于') + maxMinArr[1] + (Number(value) ? '位数！' : '个字母!');
        }

        // 匹配验证规则
        if (rule || isMaxMin) {
          isTrue = !isMaxMin ? (isFn ? errorText = rule(value, item) : !rule[0].test(value)) : false;
          ...
          errorText = !isMaxMin ? (errorText || rule[1]) : '';

          // 获取自定义必填项提示文本
          if (thisVer === 'required') {
            errorText = othis.attr('lay-reqText') || othis.attr('lay-reqtext') || othis.attr('placeholder') || errorText;
          }

          if (isMaxMin) {
            errorText = maxMinTips;
            if (vers.indexOf('number') !== -1) {
              isTrue = maxMinArr[0].toLowerCase() === 'max' ? Number(value) > Number(maxMinArr[1]) : Number(value) < Number(maxMinArr[1]);
            } else {
              isTrue = maxMinArr[0].toLowerCase() === 'max' ? value.length > Number(maxMinArr[1]) : value.length < Number(maxMinArr[1]);
            }
          }
          ...
          if (isTrue && (
            ...
            //全局设置提示样式
            verType = options.allverType || verType;
            // 提示层风格
            if (verType === 'tips') {
            ...
            else if (verType === 'toast') {
              layui.toast.error({message: errorText});
            }
